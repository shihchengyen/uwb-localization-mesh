#!/usr/bin/env python3
"""
Follow-Me Audio Client (RPi)

Receives synchronized audio commands via MQTT and plays audio.
This is the RPi-side audio player that responds to the follow-me audio server.

Usage:
    python follow-me_audio_client.py --id 0 --wav crazy-carls-brickhouse-tavern.wav --broker localhost
...
from packages.uwb_mqtt_client.config import MQTTConfig
...
class FollowMeAudioClient:
    """
    RPi audio player that receives and executes synchronized audio commands.
    """
    ...
    def __init__(self, rpi_id: int, wav_file: str, broker_ip: str = DEFAULT_BROKER_IP):
        ...
        self.config = MQTTConfig(
            ...
            client_id=f"audio_client_rpi_{rpi_id}"
        )
        ...
        print(f"🎵 RPi {rpi_id} Follow-Me Audio Client Ready")
        ...
        try:
            if command == "start":
                if not self.is_playing:
                    ...
                    print(f"🎵 STARTED at {self.current_volume}%")
    ...
    def run(self):
        ...
        try:
            print(f"🎧 RPi {self.rpi_id} Follow-Me Audio Client running...")
            ...
            print(f"\n👋 Shutting down RPi {self.rpi_id} Follow-Me Audio Client...")
        finally:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Follow-Me Audio Client (RPi)")
    ...
    client = FollowMeAudioClient(args.id, args.wav, args.broker)
    client.run()
